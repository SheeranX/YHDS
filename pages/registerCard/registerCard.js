...
Page({
    ...
    request.http(params).then(
      res => {
        ...
        if (code == 0) {
          ...
          var ticketCardList = res.ticketCardList;
          var item = null;
          for (var i in ticketCardList) {
            if (ticketCardList[i].checked == true) {
              item = ticketCardList[i];
              break;
            }
          }
          console.log(item)
  ...
  InputInfo: function() {
    ...
    let _this = this;
    if (cardInfo == null || cardInfo.ticketId == null) {
      wx.showToast({
        title: '请选择需要购买的会员卡!',
        icon: 'none',
        duration: 2000
    ...
    if (peopleFlag) {
      ...
      request.http(params).then(
        res => {
          var code = res.code;
          if (code == 0) {
            let cardOrderNo = res.cardOrderNo;
            let params2 = {
                  ...
                  wx.requestPayment({
                    ...
                    'success': function(res) {
                      wx.showModal({
                        content: '办理成功',
                        cancelText: '回主页',
                        confirmText: _this.data.isAdd?"去赠送":'去查看',
                        success: function(res) {
                          if (res.confirm) {
                            _this.data.isAdd?
                            wx.redirectTo({
                                url: '../personnel/giveFriends/giveFriends?cardOrderNo=' + cardOrderNo,
                            }):wx.redirectTo({
